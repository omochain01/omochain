#include <omo/types/TypeParser.hpp>
#include <fstream>
#include <iomanip>
...
using std::string;
namespace omo { using types::SimpleSymbolTable; }
...
string generate_wren( const omo::types::Struct& s, omo::types::SimpleSymbolTable& symbols ) {
...
void generate_wren( omo::types::SimpleSymbolTable& ss, const char* outfile ) {
  //for( const auto& s : ss.order ) 
}

void generate_hpp( omo::types::SimpleSymbolTable& ss, const char* outfile ) {
   struct FakeField { std::string type; std::string name; };
   auto arrays_to_vectors = [](omo::types::Field f) {
   ...
   out << "#pragma once\n";
   out << "namespace omo { namespace types {\n";
   for( const auto& s : ss.order ) {
   ...
   for( const auto& s : ss.order ) {
      ...
      const auto& st = ss.structs[s];
      if( st.base.size() ) {
        out << "FC_REFLECT_DERIVED( omo::types::" << s << ", (omo::types::" << st.base <<"), ";
      } else  {
        out << "FC_REFLECT( omo::types::" <<  std::setw(33) << s << ", ";
      }
      for( const auto& f : st.fields ) {
         out <<"("<<f.name<<")";
      }
...
int  count_fields( omo::types::SimpleSymbolTable& ss, const omo::types::Struct& st ) {
   if( st.base.size() )
...
int main( int argc, char** argv ) {
  try {
     ...
     omo::types::SimpleSymbolTable ss;
     ss.parse(in);
